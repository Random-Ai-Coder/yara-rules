...
rule PREVENT_JAVASCRIPT_UPLOAD {
    meta:
        type = "embedded javascript in documents"
        description = "Prevent file upload with JavaScript code with java script embedded documents"
        bayshore_ix = 158
    strings:
        $js = "eval(" wide
        $script = "<script" wide
        $javascript = "javascript:" nocase wide
        $onload = "onload=" wide
        $unescape = "unescape(" wide
        $atob = "atob(" wide
    condition:
        any of them
}
